...
class SEBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # y = F.softmax(self.fc2(y), dim=1).view(b, c, 1, 1)
        # Reweighting
...
class VitGuidanceHead(BaseDecodeHead):
    def __init__(self,
        ...
        if self.inter_transform_type == 'linear':
            # 将输入特征图转换。nn.Conv2d(input_channel,outout_channel,kernel_size)
            self.feature_transforms = nn.Conv2d(base_channels * 4, self.channelsDefault[0], 1)
            self.conv3 = nn.Conv2d(base_channels * 4, self.channelsDefault[1], 1)
            self.relu = nn.ReLU()
            self.conv4 = nn.Conv2d(base_channels * 8, self.channelsDefault[2], 1)
            self.convstage4 = nn.Conv2d(self.channelsDefault[2], self.channelsDefault[2], 1)
            self.convstage3 = nn.Conv2d(self.channelsDefault[1], self.channelsDefault[1], 1)

            self.se320 = SEBlock(self.channelsDefault[1])
            self.se512 = SEBlock(self.channelsDefault[2])
        ...
        self.convstage4.apply(self._weights_kaiming)
        self.convstage3.apply(self._weights_kaiming)
        self.conv4.apply(self._weights_kaiming)
        self.conv3.apply(self._weights_kaiming)
    ...
    def forward(self, inputs,decoder_feature,decoder_seg_logits):
        ...
        n1, c1, h1, w1 = x_guidance_feature[1][1].shape
        n2, c2, h2, w2 = x_guidance_feature[1][2].shape
        device = x_guidance_feature[1][1].device
        mean, std = 0, 1
        # 生成一个高斯噪声张量：均值为 0，标准差为 1，形状为 (N, 1, H, W)
        gaus_noise_1 = torch.normal(mean=mean, std=std, size=(n1, 1, h1, w1)).to(device)
        gaus_noise_2 = torch.normal(mean=mean, std=std, size=(n2, 1, h2, w2)).to(device)
        gaused_fea_1 = torch.add(x_guidance_feature[1][1], gaus_noise_1)
        gaused_fea_2 = torch.add(x_guidance_feature[1][2], gaus_noise_2)
        ...
        if self.inter_transform_type == 'linear':
            ...
            feat_s.append(
                    self.se512(self.convstage4(self.relu(self.conv4(gaused_fea_1)))))  # x5,  4,512,32,16 stride=32
            feat_s.append(
                    self.se320(self.convstage3(self.relu(self.conv3(gaused_fea_2)))))  # x5,  4,512,32,16 stride=32
